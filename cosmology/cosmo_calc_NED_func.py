#!/usr/bin/env python
# Taken from http://www.astro.ucla.edu/~wright/CC.python

import sys
from math import *
import argparse

def main(z, H0=67.4, WM=0.32, WV=0.68):
...
# comoving volume computation
  ...
  '''
  return DA_Mpc,kpc_DA,DL_Mpc


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cosmology calculator")
    parser.add_argument("z", help="Redshift", type=float)
    parser.add_argument("--H0", default=70., help="Hubble constant", type=float)
    parser.add_argument("--WM", default=0.3, help="Omega matter", type=float)
    parser.add_argument("--WV", default=0.7, help="Omega Lambda", type=float)
    args = parser.parse_args()
    z = args.z
    H0 = args.H0
    WM = args.WM
    WV = args.WV
    DA_Mpc,kpc_DA,DL_Mpc = main(z, H0=H0, WM=WM, WV=WV)
    print("DA = %.3f  Mpc" % (DA_Mpc))
    print("Scale = %.3f  kpc/arcsec" % (kpc_DA))
    print("DL = %.3f  Mpc" % (DL_Mpc))
