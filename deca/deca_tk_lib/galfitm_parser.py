...
def to_deca_format(fileName, CompNames, ComponentFunctions, galfitm=False):
              ...
              for par in ComponentFunctions:
                if par.split(':')[0]==func_name:
                    ...
                    for i in range(len(Component_block)):
                      if par_num+')' in Component_block[i] and 'Sersic' not in Component_block[i]:
                        if Component_block[i].lstrip().startswith('1)') and str(func_name)!='sky':
                          stri = Component_block[i].lstrip().split()
                          par_var_x = list(np.array(stri[1].split(','), float))
                          par_min_var = ''
                          par_max_var = ''
                          FinalParameters_load.update({str(func_name)+':'+str(1)+':'+'x_cen'+'/'+str(Dupl_Functions[func_name]):[par_var_x,par_min_var,par_max_var]})     
                        elif Component_block[i].lstrip().startswith('2)') and str(func_name)!='sky':
                          stri = Component_block[i].lstrip().split()
                          par_var_y = list(np.array(stri[1].split(','), float))
                          par_min_var = ''
                          par_max_var = ''
                          FinalParameters_load.update({str(func_name)+':'+str(2)+':'+'y_cen'+'/'+str(Dupl_Functions[func_name]):[par_var_x,par_min_var,par_max_var]}) 
                        else:
                          stri = Component_block[i].lstrip().split()
                          par_var = list(np.array(stri[1].split(','), float))
                          par_min_var = ''
                          par_max_var = ''
            ...
            for key, value in FinalParameters_load.items() :
              TMP_PARS.append(key+':'+" ".join(str(x) for x in value[0])+','+str(value[1])+','+str(value[2]))
