...
# Taken from https://github.com/keflavich/image_tools/blob/master/image_tools/radialprofile.py
...
mask_non_astropy = np.ones_like(np.array(data,dtype=float),dtype=bool)
mask_astropy = np.zeros_like(np.array(data,dtype=float),dtype=bool)


m0 = 27.
pix2sec = 0.396
bin_centers,radial_prof = azimuthalAverage(data,returnradii=True,mask=mask_non_astropy)
plt.plot(bin_centers,m0-2.5*log10(radial_prof)+5.*log10(pix2sec),'o')
ylim(26,18)
...
print m0-2.5*log10(std)+5.*log10(pix2sec)
...
print initial_guess.define_bulge_dom_region(r,I,m0,pix2sec)

            
plt.plot(r,m0-2.5*log10(I)+5.*log10(pix2sec),'*')
