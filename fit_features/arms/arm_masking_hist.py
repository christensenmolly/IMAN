...
def get_inten_along_ellipse(ellipse, data, new_mask, mask=None): #### Added mask
    ...
    inds = list(outliers_iqr(intensity)[0])
    ...
    for i in range(len(exc_anomaly)):
        ...
        if (data[iiy][iix]>I_backg_max and data[iiy][iix]>0.) or (I_res >=0.25 and I_res < 1.0) or i in inds: #median+1.*std:# or data[iiy][iix]<=median-1.*std:   # if i in inds:
            new_mask[iiy][iix] = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", help="Name of output model",
                        type=str, default='mask_spirals.fits') #'azim_model.fits'
