...
# ------------------------------------------------------------------------------
""" % (input_image, sigma_image, psf_image, mask_image, nx, ny, nx, ny)# 3*n_psf, 3*n_psf)
...
def run_galf(Z, L0, h, xc, yc, z0, sigma_image, psf_image, mask_image):
  try:
    #h=40.
    #z0 = 30.
    #print(Z, L0, h, xc, yc, z0)
    if float(xc)==0.:        
        I0z = L0*2.*h / ((np.fabs(1.)/h) * kn(1,np.fabs(1.)/h) * np.exp(-np.fabs(Z)/z0) ) # (1.0 / np.cosh(1.*np.fabs(Z)/(2.*z0)) )**2 )
    else:
        I0z = L0*2.*h / ((np.fabs(xc)/h) * kn(1,np.fabs(xc)/h) * np.exp(-np.fabs(Z)/z0) ) # (1.0 / np.cosh(1.*np.fabs(Z)/(2.*z0)) )**2 )
    #print(xc,yc,Z, L0, -2.5*math.log10(abs(I0z)))
    #I0d * (np.fabs(r-x_c)/h)*kn(1,np.fabs(r-x_c)/h)  * (1.0 / np.cosh(n*np.fabs(z-z_c)/(2.*z0)) )**(2./n)
      
    crea_galfit_input('radial_line.fits', sigma_image, psf_image, mask_image, xc, -Z, -2.5*math.log10(abs(I0z)), z01=2.*z0, h1=h,  mu0d2=None, z02=None, h2=None) #### TODO: ADD module!
...
def single_disc_fit(Z, L0, h, xc, yc, z0, input_image, sigma_image, psf_image, mask_image, ron, gain):
    if code=='imfit':
        # Levenberg-Marquardt:
        status = run_imf(L0, h, xc, sigma_image, psf_image, mask_image, ron, gain)
        if status==0:
            L00,H,chi2 = read_imf('bestfit_parameters_imfit_lm.dat')
        else:
            L00 = float('nan'); H = float('nan')
    if code=='galfit':
        status = run_galf(Z, L0, h, xc, yc, z0, sigma_image, psf_image, mask_image)
        if status==0:
...
def fit_each_hor_slice(Z, image_data, mask_data, sigma_data, psf_image, z_min_ind, z_max_ind, x_min_ind, x_max_ind, xc, yc, ron, gain, L0=None, h=None, z0=None, y0=None, L02=None, N2=None, z02=None,  double_disc=False, side='1', h_given=None, write_prof=True):
    ...
    if side=='2' or side=='3':
        L0,h = single_disc_fit(Z, L0, h, xc-x_min_ind, yc, z0, input_image, sigma_image, psf_image, mask_image, ron, gain)
    else:
        L0,h = single_disc_fit(Z, L0, h, xc-x_min_ind, yc, z0, input_image, sigma_image, psf_image, mask_image, ron, gain)
...
def do_each_bin_z(Z, image_data, sigma_data, mask_data, psf_image, z_min_ind, z_max_ind, x_min_ind, x_max_ind, xc, yc, z0, break_radii=None, error_est=True, ron=None, gain=None, side='1', h_given=None):
    ...
    L0,h,I_image,zz = fit_each_hor_slice(Z, image_data, mask_data, sigma_data, psf_image, z_min_ind, z_max_ind, x_min_ind, x_max_ind, xc, yc, ron, gain, side=side, z0=z0, h_given=h_given)
    ...
    if error_est:
        ...
        for k in slice_list:
            p1,p2,p3,p4 = fit_each_hor_slice(Z, image_data, mask_data, sigma_data, psf_image, k, k+1, x_min_ind, x_max_ind, xc, yc, ron, gain, L0, h, side=side, z0=z0, h_given=h_given, write_prof=False)
            L00.append(p1)
            H00.append(p2)
    ...
    if break_radii is not None:
        ...
        for k in range(len(x_nodes)-1):
            ...
            L000,h00,I_image,zz = fit_each_hor_slice(Z, image_data, mask_data, sigma_data, psf_image, z_min_ind, z_max_ind, x_nodes[k], x_nodes[k+1], xc, yc, ron, gain, side=side, z0=z0, h_given=h_given)
            #print('\t',L000,h00)
            ...
            if error_est:
                ...
                for kk in slice_list:
                    p1,p2,p3,p4 = fit_each_hor_slice(Z, image_data, mask_data, sigma_data, psf_image, kk, kk+1, x_nodes[k], x_nodes[k+1], xc, yc, ron, gain, L0, h, side=side, z0=z0, h_given=h_given, write_prof=False)
                    L00.append(p1)
                    H00.append(p2)
...
def main(input_image, mask_image=None, weight_image=None, psf_image=None, R_in_l=0., R_out_l=None, R_in_r=0., R_out_r=None, z_in_dn = 0., z_out_dn=None, z_in_up=0., z_out_up=None, break_radii=None, bin_type='linear', side='1', error_est=False, z0=5., h_given=None):
    # Sides:
    # 1 - up, right
    # 2 - up, left
    # 3 - bottom, left
    # 4 - bottom, right
    # z0 is hz, i.e real z0 = 2hz
    ...
    ff = open('radial_profiles_%s_%s.txt' % (side, 'single'),'w')
    ff.close()
    ...
    for k in range(0, len(ind_z_bins)-1):
        ...
        [L0,L0_err,h,h_err],[L01,L01_err,h1,h1_err,Rbr1],[L02,L02_err,h2,h2_err,Rbr2],[L03,L03_err,h3,h3_err] = do_each_bin_z(Z, image_data, sigma_data, mask_data, psf_image_cut, ind_z_bins[k], ind_z_bins[k+1], x_min_ind, x_max_ind, ds9_to_np(xc), ds9_to_np(yc), z0, break_radii=break_radii, error_est=error_est, side=side, h_given=h_given)
...
if __name__ == "__main__":      
    ...
    main(input_image, mask_image=mask_image, weight_image=weight_image, psf_image='psf.fits', R_in_l=R_in_l, R_out_l=R_out_l, R_in_r=R_in_r, R_out_r=R_out_r, z_in_dn = z_in_dn, z_out_dn=z_out_dn, z_in_up=z_in_up, z_out_up=z_out_up, break_radii=None, bin_type='linear', side='1', error_est=False, z0=z0, h_given = h)  
'''
'''
...
if __name__ == "__main__":      
    ...
    main(input_image, mask_image=mask_image, weight_image=weight_image, psf_image='psf.fits', R_in_l=R_in_l, R_out_l=R_out_l, R_in_r=R_in_r, R_out_r=R_out_r, z_in_dn = z_in_dn, z_out_dn=z_out_dn, z_in_up=z_in_up, z_out_up=z_out_up, break_radii=None, bin_type='linear', side='1', error_est=False, z0=z0)  
'''
'''
# NGC4302
if __name__ == "__main__":      
