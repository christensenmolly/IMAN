...
def crea_galfit_input_tripple(input_image, sigma_image, psf_image, mask_image, xc, yc, mu0d1, z01=None,  mu0d2=None, z02=None, mu0d3=None, z03=None):
    if sigma_image is None:
        sigma_image = 'none'
...
def run_galf_tripple(L01, n1, z01, y0, L02, n2, z02, L03, n3, z03, sigma_image, psf_image, mask_image, ron, gain):
  try:
    crea_galfit_input_tripple(input_image, sigma_image, psf_image, mask_image, 0.5, y0, -2.5*math.log10(L01*(2.*10.)), z01=2.*z01,  mu0d2=-2.5*math.log10(L02*(2.*10.)), z02=2.*z02, mu0d3=-2.5*math.log10(L03*(2.*10.)), z03=2.*z03)  
...
def tripple_disc_fit(L01, n1, z01, y0, L02, n2, z02, L03, n3, z03, n_fixed, sigma_image, psf_image, mask_image, ron, gain):
    z01 = np.fabs(z01)
    z02 = np.fabs(z02)
    z03 = np.fabs(z03)
    if code=='imfit':
        ...
        status = run_galf_tripple(L01, n1, z01, y0, L02, n2, z02, L03, n3, z03, sigma_image, psf_image, mask_image, ron, gain)
        if status == 0:
...
def fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed='fixed', L0=None, N=None, z0=None, y0=None, L02=None, N2=None, z02=None, L03=None, N3=None, z03=None,  disc_comps = 'single'):
    ...
    if disc_comps=='single':
        ...
        return L0,Y0,N,Z0,I_no_mask,r_no_mask
        ...
        '''
        if L02 is None:
            L01,L02,z01,z02 = find_double_disc_guess(r_no_mask, I_no_mask, y0)  #### WARNING: DOES NOT WORK PROPERLY!!!
        else:
            L01 = L0
            z01 = z0
    ...
    if disc_comps=='tripple':
            I_1 = disk_edge_soph(rr, 2.*10.*L01, Z01, y0)
            I_2 = disk_edge_soph(rr, 2.*10.*L02, Z02, y0)
            I_sum = I_1 + I_2
            I_3 = I_image - I_sum
...
def prepare_slice_images(I_image, I_sigma, I_mask):
    ...
    mask_image = 'mask_line.fits'
    #### Create slice images END ###   
    return sigma_image,mask_image



def fit_one_disc(r_no_mask, I_no_mask, I_image, I_sigma, I_mask, n_fixed, psf_image, ron, gain):   
    I0 = max(I_no_mask)

    sigma_image,mask_image = prepare_slice_images(I_image, I_sigma, I_mask)
...
def fit_each_vert_slice_double(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed='fixed', error_est = False):
    ...
    # Thick disc:
    rr_2, I_image_2, I_mask_2, I_sigma_2, r_no_mask_2, I_no_mask_2 = crea_vert_average_cut(image_data, mask_data, sigma_data, x_min_ind, x_max_ind, y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up)     
    
    L02,Y02,N2,Z02 = fit_one_disc(r_no_mask_2, I_no_mask_2, I_image_2, I_sigma_2, I_mask_2, n_fixed, psf_image, ron, gain)
    
 
    # Thin disc
    rr, I_image, I_mask, I_sigma, r_no_mask, I_no_mask = crea_vert_average_cut(image_data, mask_data, sigma_data, x_min_ind, x_max_ind, y_min_ind_dn, y_max_ind_up, y_min_ind_up, y_max_ind_up)     #### TODO:CHECK!!!   
    #print(y_min_ind_dn, y_max_ind_up, y_min_ind_up, y_max_ind_up)

    
    Ltot = np.trapz(I_image, x=rr)
    I_image_1 = I_image - disk_edge_soph(rr, 2.*10.*L02, Z02, Y02) # TODO: Y02 should be real y0!!!!!!!!!!!!!!!!!!!!!!
    ...
    if L_1/Ltot<0.01: #### TODO: CHECK
        # Just one disc
        L01,Y01,N1,Z01 = fit_one_disc(r_no_mask, I_no_mask, I_image, I_sigma, I_mask, n_fixed, psf_image, ron, gain)
        ...
        if error_est:
            ...
            for k in slice_list:
                # Thin disc
                rr, I_image, I_mask, I_sigma, r_no_mask, I_no_mask = crea_vert_average_cut(image_data, mask_data, sigma_data, k, k+1, y_min_ind_dn, y_min_ind_dn+2, y_min_ind_dn+1, y_max_ind_up)     #### TODO:CHECK!!!   
                
                p1,p2,p3,p4 = fit_one_disc(r_no_mask, I_no_mask, I_image, I_sigma, I_mask, n_fixed, psf_image, ron, gain)
                L001.append(p1)
                Y001.append(p2)
                NN1.append(p3)
                Z001.append(p4)                   
        ...
        L01,Y01,N1,Z01 = fit_one_disc(r_no_mask, I_no_mask, I_image_1, I_sigma, I_mask, n_fixed, psf_image, ron, gain)
        ...
        if error_est:
            ...
            for k in slice_list:
                # Thick disc:
                rr_2, I_image_2, I_mask_2, I_sigma_2, r_no_mask_2, I_no_mask_2 = crea_vert_average_cut(image_data, mask_data, sigma_data, k, k+1, y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up)     
                
                p1,p2,p3,p4 = fit_one_disc(r_no_mask_2, I_no_mask_2, I_image_2, I_sigma_2, I_mask_2, n_fixed, psf_image, ron, gain)
                L002.append(p1)
                Y002.append(p2)
                NN2.append(p3)
                Z002.append(p4)                
                ...
                I_image_1 = I_image - disk_edge_soph(rr, 2.*10.*p1, p4, p2) # TODO: Y02 should be real y0!!!!!!!!!!!!!!!!!!!!!!    
                
                p1,p2,p3,p4 = fit_one_disc(r_no_mask, I_no_mask, I_image_1, I_sigma, I_mask, n_fixed, psf_image, ron, gain)
                L001.append(p1)
                Y001.append(p2)
                NN1.append(p3)
                Z001.append(p4)                   
...
def do_each_bin_x(R, image_data, sigma_data, mask_data, psf_image, x_min_ind, x_max_ind, y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, disc_comps='single', error_est=True, ron=None, gain=None, n_fixed='fixed', side=1):
    ...
    L0,Y0,N,Z0,I_image,rr = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind, y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed)
    ...
    if error_est:
        ...
        for k in slice_list:
            p1,p2,p3,p4,p5,p6 = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, k, k+1,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed, L0, N, Z0, Y0)
            L00.append(p1)
            Y00.append(p2)
            NN.append(p3)
            Z00.append(p4)
    ...
    if disc_comps=='double1':
        # Double disc fit:
        L01,N1,Z01,L02,N2,Z02 = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed, L0, N, Z0, Y0, disc_comps='double1')
        ...
        if error_est:
            ...
            for k in slice_list:
                p1,p2,p3,p4,p5,p6 = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, k, k+1,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed, L01, N1, Z01, Y0, L02, N2, Z02, disc_comps='double1')
                #exit()
                L001.append(p1)
                NN1.append(p2)
                Z001.append(p3)
    ...
    if disc_comps=='double': 
        L01,L01_err,N1,N1_err,Z01,Z01_err,L02,L02_err,N2,N2_err,Z02,Z02_err = fit_each_vert_slice_double(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed='fixed', error_est =error_est)
        
 

    if disc_comps=='tripple':
        # Tripple disc fit:
        L01,N1,Z01,L02,N2,Z02,L03,N3,Z03 = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, x_min_ind, x_max_ind,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed, L0, N, Z0, Y0, disc_comps='tripple')
        ...
        if error_est:
            ...
            for k in slice_list:
                p1,p2,p3,p4,p5,p6,p7,p8,p9 = fit_each_vert_slice(image_data, mask_data, sigma_data, psf_image, k, k+1,  y_min_ind_dn, y_max_ind_dn, y_min_ind_up, y_max_ind_up, ron, gain, n_fixed, L01, N1, Z01, Y0, L02, N2, Z02, L03, N3, Z03, tripple_disc=tripple_disc)
                #exit()
                L001.append(p1)
                NN1.append(p2)
                Z001.append(p3)
...
def main(input_image, mask_image=None, weight_image=None, psf_image=None, R_in_l=0., R_out_l=None, R_in_r=0., R_out_r=None, z_in_dn = 0., z_out_dn=None, z_in_up=0., z_out_up=None, bin_type='linear', n_fixed='fixed', side='1', error_est=True, disc_comps='single'):
    # Sides:
    # 1 - right
    # 2 - left
    ...
    for k in range(0, len(ind_x_bins)-1):
            ...
            [L0,L0_err,Y0,Y0_err,N,N_err,Z0,Z0_err],[L01,L01_err,N1,N1_err,Z01,Z01_err],[L02,L02_err,N2,N2_err,Z02,Z02_err],[L03,L03_err,N3,N3_err,Z03,Z03_err] = do_each_bin_x(R, image_data, sigma_data, mask_data, psf_image_cut, ind_x_bins[k], ind_x_bins[k+1], z_min_ind_dn, z_max_ind_dn, z_min_ind_up, z_max_ind_up, disc_comps=disc_comps, error_est=error_est, n_fixed=n_fixed, side=side)
...
if __name__ == "__main__":    
    ...
    shutil.move('vertical_fits_1.dat', 'vertical_fits_1_double.dat')
    ...
    shutil.move('vertical_fits_1.dat', 'vertical_fits_1_single.dat')
...
if __name__ == "__main__":    
    ...
    main(input_image, mask_image=mask_image, weight_image=weight_image, psf_image=None, R_in_l=R_in_l, R_out_l=R_out_l, R_in_r=R_in_r, R_out_r=R_out_r, z_in_dn = z_in_dn, z_out_dn=z_out_dn, z_in_up=z_in_up, z_out_up=z_out_up, bin_type='linear', n_fixed=n_fixed, side='1', error_est=False, disc_comps='single')
...
if __name__ == "__main__":    
    ...
    main(input_image, mask_image=mask_image, weight_image=weight_image, psf_image=None, R_in_l=R_in_l, R_out_l=R_out_l, R_in_r=R_in_r, R_out_r=R_out_r, z_in_dn = z_in_dn, z_out_dn=z_out_dn, z_in_up=z_in_up, z_out_up=z_out_up, bin_type='linear', n_fixed=n_fixed, side='1', error_est=False, disc_comps='double')
    shutil.move('vertical_fits_1.dat', 'vertical_fits_1_double.dat')
