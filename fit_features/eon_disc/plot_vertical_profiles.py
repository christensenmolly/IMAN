...
def read_profiles(profiles_file):
    ...
    for cut in cuts:
      if cut!='':
        ...
        Z.append(zz)
        I.append(II)
    return np.array(radius),np.array(Z),np.array(I)
    

def main(profiles_file, results_file, psf_image=None, units = ['arcsec',0.75], mode='single'):
    if psf_image is not None:
        kernel = fit_eon_vertical.psf_slice(psf_image, axis='yaxis')
        kernel = kernel/np.sum(kernel)
    else:
        kernel = None
        psf_image = 'none'
    
    radius,Z,I = read_profiles(profiles_file)
    ...
    for k in range(len(radius)):
        mag = M0 - np.log10(I[k])       
        ax.plot(Z[k]-Y0[k], mag,'o', markersize=9,color='gray',markeredgecolor='gray')

        if mode=='single':
            #print(L0[k], Z0[k], Y0[k], N[k])
            #exit()
            #I_single = disk_edge_soph(Z[k], 2.*10.*L0[k], Z0[k], Y0[k], N[k])
            I_single = run_gulf_single(psf_image, Y0[k], len(Z[k]), M0-2.5*math.log10(2.*10.*L0[k]), 2.*Z0[k], 10., M0)
            
            #2.*10.*L0[k] * (np.fabs(R[k]-251.)/10.)*kn(1,np.fabs(R[k]-251.)/10.)
            
            I0dc = 1. #(2.*10.*L0[k]) / ((np.fabs(R[k]-251.)/40.)*kn(1,np.fabs(R[k]-251.)/40.))
            #print()
            # run_gulf_single(psf_image, r, xc, yc, nx, ny, mu0d, z0, h)
            #I_single = run_gulf_single(psf_image, R[k], 251., Y0[k], 501, 251, M0-2.5*math.log10(I0dc), 2.*Z0[k], 40., M0) #### WORKS FINE !!!!!!!!!!!!!!!!!!!!!
            #I_single = I_single*2.*40.*L0[k]/(np.max(I_single))
            #print(I_single)
            #exit()
            
            #if kernel is not None:
            #    I_single = signal.fftconvolve(I_single, kernel, mode='same')
            
            mag_single = M0 - np.log10(I_single)
            #print(mag_single,mag )
            #exit()
            ax.plot(Z[k]-Y0[k], mag_single, '-', color='red')
        elif mode=='double':
            ...
            #if kernel is not None:
            #    I_sum = signal.fftconvolve(I_sum, kernel, mode='same')
            #    I_1 = signal.fftconvolve(I_1, kernel, mode='same')
            #    I_2 = signal.fftconvolve(I_2, kernel, mode='same')   
            
            I_1 = run_gulf_single(psf_image, Y0[k], len(Z[k]), M0-2.5*math.log10(2.*10.*L01[k]), 2.*Z01[k], 10., M0)
            I_2 = run_gulf_single(psf_image, Y0[k], len(Z[k]), M0-2.5*math.log10(2.*10.*L02[k]), 2.*Z02[k], 10., M0)
            I_sum = run_gulf_double(psf_image, Y0[k], len(Z[k]), M0-2.5*math.log10(2.*10.*L01[k]), 2.*Z01[k], 10., -2.5*math.log10(2.*10.*L02[k]), 2.*Z02[k], 10., M0)
            ...
            I_1 = disk_edge_soph(Z[k], 2.*10.*L01[k], Z01[k], Y0[k], N1[k])
            I_2 = disk_edge_soph(Z[k], 2.*10.*L02[k], Z02[k], Y0[k], N2[k])
            I_3 = disk_edge_soph(Z[k], 2.*10.*L03[k], Z03[k], Y0[k], N3[k])
            I_sum = I_1 + I_2 + I_3
            if kernel is not None:
                I_sum = signal.fftconvolve(I_sum, kernel, mode='same')
                I_1 = signal.fftconvolve(I_1, kernel, mode='same')
                I_2 = signal.fftconvolve(I_2, kernel, mode='same')   
                I_3 = signal.fftconvolve(I_3, kernel, mode='same')   
            ...
            ax.plot(Z[k]-Y0[k], mag_1, '--', color='blue')            
            ax.plot(Z[k]-Y0[k], mag_2, '--', color='red')
            ax.plot(Z[k]-Y0[k], mag_3, '--', color='lime')
            
        min_mag.append(np.nanmin(mag))
        max_mag.append(np.nanmax(mag))
        max_z.append(np.nanmax(np.fabs(Z[k]-Y0[k])))
        M0 = M0 - 2
    ...
    ax.set_xlim(-np.nanmax(max_z), np.nanmax(max_z))
...
main('vertical_profiles_1.txt', 'vertical_fits_1_single.dat', psf_image='psf.fits', mode='single')
#main('vertical_profiles_1.txt', 'vertical_fits_1_double.dat', psf_image='psf.fits', mode='double')
#main('vertical_profiles_1.txt', 'vertical_fits_1.dat', psf_image='psf.fits', mode='tripple')