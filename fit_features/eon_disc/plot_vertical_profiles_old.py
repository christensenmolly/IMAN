...
def run_gulf_single(psf_image, yc, ny, mu0d, z0, h):
...
def run_gulf_double(psf_image, yc, ny, mu0d1, z01, h1, mu0d2, z02, h2):
    if psf_image is None:
        psf_image = 'none'
        n_psf=1
    else:
...
def main(profiles_file, results_file, psf_image=None, units = ['arcsec',0.75], mode='single'):
    ...
    for k in range(len(radius)):
        ...
        if mode=='single':
            ...
            I_single = run_gulf_single(psf_image, Y0[k], len(Z[k]), -2.5*math.log10(2.*10.*L0[k]), 2.*Z0[k], 10.)
            #print(I_single)
            #exit()
            ...
            I_1 = run_gulf_single(psf_image, Y0[k], len(Z[k]), -2.5*math.log10(2.*10.*L01[k]), 2.*Z01[k], 10.)
            I_2 = run_gulf_single(psf_image, Y0[k], len(Z[k]), -2.5*math.log10(2.*10.*L02[k]), 2.*Z02[k], 10.)
            I_sum = run_gulf_double(psf_image, Y0[k], len(Z[k]), -2.5*math.log10(2.*10.*L01[k]), 2.*Z01[k], 10., -2.5*math.log10(2.*10.*L02[k]), 2.*Z02[k], 10.)
...
main('vertical_profiles_1.txt', 'vertical_fits_1.dat', psf_image='psf.fits', mode='single')
