...
def main(input_region_file, input_image, output_region_file):
        #print 'Converting the regions to fk5...'
        hdulist = pyfits.open(input_image)
        data = hdulist[0].data
        header = hdulist[0].header
        ...
        for Line in f_tmp:
            if 'polygon' in Line:
                ...
                for k in range(0,len(coords)-1,2):
                    pixcrd.append([int(float(coords[k])),int(float(coords[k+1]))])
                ...
                for k in range(len(world)):
                    polygon = str(world[k][0]) + ',' + str(world[k][1])
                    if k==0:
                        f_reg.write('polygon(%s,' % (polygon) )
                    elif k>0 and k<len(world)-1:
                        f_reg.write('%s,' % (polygon) )
                    else:
                        f_reg.write('%s)\n' % (polygon) )    
            if 'ellipse' in Line:
                        params = Line.split(",")
                        cen = Point(float(params[0].split('(')[1]),float(params[1]))
                        ellA = float(params[2])
                        ellB = float(params[3])
                        ...
                        if ellA < ellB:
                            ellA, ellB = ellB, ellA
                            ellPA += 90
                        pixcrd = np.array([[cen.x,cen.y]], np.float_)
                        world = w.wcs_pix2world(pixcrd, 1)    
                        f_reg.write("ellipse(%f,%f,%f\",%f\",%f)\n" % (world[0][0], world[0][1],ellA*pix2sec, ellB*pix2sec, ellPA))
            if 'circle(' in Line:
