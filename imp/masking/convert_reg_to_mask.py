...
def mask1(input_image, reg_file, output_image=None, output_mask=None, mask_value=1, verbosity=True, mask_DN=None):
...
def mask(input_image, reg_file, output_image=None, output_mask=None, mask_value=1, mask_DN=None, verbosity=True):
                    ...
                    for line in f:
						  ...
						  float(line.split("(")[-1].split(",")[1]))
                            ...
                            # Detect polygon regions and add them to the path list
                                ...
                                param = list(param)
                                param2 = [None]*(int(len(param)/2))
                                for i in range(0,len(param2)):  param2[i] = (int(param[2*i+1]),int(param[2*i])) #(int(param[2*i]),int(param[2*i+1])) 
                                param2.append(param2[0])
                                codes = []
                                codes.append(Path.MOVETO)
                                for i in range(1,len(param2)-1):  codes.append(Path.LINETO)
                ...
                if True:                     
                      for kk in range(len(paths)):
                       try:
                        ...
                        x, y = np.meshgrid(np.arange(xmin,xmax), np.arange(ymin,ymax)) # make a canvas with coordinates
                        ...
                        new_dat = 1*grid.reshape(ymax-ymin,xmax-xmin)

                        img2[ymin:ymax,xmin:xmax] = img2[ymin:ymax,xmin:xmax] + new_dat
                      ...
                      img2[img2>0] = 1 # WARNING: Can be omitted!
...
if __name__ == '__main__':
    ...
    mask(input_image, reg_file, output_image=output_image, output_mask=output_mask, mask_value=mask_value, verbosity=True, mask_DN=mask_DN)
