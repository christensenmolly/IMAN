...
def do_offseting(input_region_file, output_region_file, offset_size=1., offset_pix=0., xc=None, yc=None, system='image'):
              #offset = [1.5,1.]
              ...
              for Line in f_tmp:
                        ...
                        if 'polygon' in Line:
                          ...
                          Offset = 0
                          if offset_size!=1.:
                            Offset = max([abs(max(X)-min(X)), abs(max(Y)-min(Y))]) * abs(float(offset_size)-1)
                          if offset_pix!=0.:
                            Offset = Offset + offset_pix
...
def main(segm_file, output_region_file, output_mask_image=None, fits_slice = 0, offset_size=1., offset_pix=0., xc=None, yc=None, system='image', ignore_value=None, verbosity=True):
            # Open the segmentation file
            hdulist = pyfits.open(segm_file)
            data = hdulist[0].data
            if ignore_value!=None:
              if verbosity: print('Ignoring masks with values larger than %f!' % (ignore_value))
            ...
            if offset_size!=1. or offset_pix!=0.:
              do_offseting('tmp.reg', output_region_file, offset_size, offset_pix, xc=xc, yc=yc)
              os.remove('tmp.reg')
            else:
...
if __name__ == '__main__':
    ...
    main(segm_file, output_region_file, output_mask_image=output_mask_image, fits_slice = fits_slice, offset_size=offset_size, offset_pix=offset_pix, xc=xc, yc=yc)
