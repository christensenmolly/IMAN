...
sys.path.append('/home/amosenko/MEGA/MyPrograms/IMAN/IMP_NEW')
...
def inner_mask(input_image, x0, y0, q, PA, max_radius=None, sigma=2., offset_size=1., offset_pix=0., min_radius=5., step = 1., FWHM=3., npixels=5, output_mask_file='inner_mask.fits', output_reg_file='inner_mask.reg'):
    '''
    DESCRIPTION:
    ...
    and use sigma clipping to mask pixels which have intensities larger than mean+sigma*std.
    ...
    for radius in radii:
        ...
        try:
            ...
            for k in range(len(RR)):
                if data[CC[k],RR[k]]>mean + sigma*std:   #sigma*mean:
                    mask[CC[k],RR[k]]=1.
        except:
    ...
    if offset_size!=1. or offset_pix!=0.:  
        convert_segm_to_region_new.main(output_mask_file, output_reg_file, output_mask_image=None, offset_size=offset_size, offset_pix=offset_pix,xc=x0,yc=y0)
    print '\t Done!'



def outer_mask(input_image, x0, y0, sma, q, PA, snr=2., offset_size=1., offset_pix=0., output_mask_file='outer_mask.fits', output_reg_file='outer_mask.reg'):
    '''
    DESCRIPTION:
    ...
    if offset_size!=1. or offset_pix!=0.:  
        ...
        #convert_segm_to_region_new.main(output_mask_file, 'tmp.reg',offset_size=offset_size, offset_pix=offset_pix,xc=None,yc=None)
        convert_segm_to_region_new.do_offseting(output_reg_file, 'tmp.reg', offset_size, offset_pix, xc=None, yc=None, system='image')
...
def main(input_image, x0, y0, sma, q, PA, snr_outer=2., sigma_inner=2., offset_size=1., offset_pix=0.):
    # Create outer mask
    outer_mask(input_image, x0, y0, sma, q, PA, snr=snr_outer, offset_size=offset_size, offset_pix=offset_pix, output_mask_file='outer_mask.fits', output_reg_file='outer_mask.reg')

    # Create final mask
    #galaxy_ellipse = [x0, y0, sma, q, PA]
    galaxy_ellipse = [[[x0, y0], sma, sma*q, PA]]
    mask_objects.add_galaxy_mask('outer_mask.fits', 'final_mask.fits', galaxy_ellipse, sma_in_factor=0., sma_out_factor=4.0)
    
    convert_segm_to_region_new.main('final_mask.fits', 'final_mask.reg')
...
#main('new-image.fits', 1376.4, 1233.7, 308.7, 0.42, -45, snr_outer=2., sigma_inner=2., offset_size=1.5, offset_pix=0.)

#python3 ~/MEGA/MOL_A/Pipeline_scripts/mask_outliers_on_galaxy.py sky_subtr_crop.fits --xcen 1150 --ycen 1146 --ell 0.33 --posang 45 --sma-max 370 --sma-min 20
#inner_mask('sky_subtr_crop.fits', 1150, 1146, 0.67, -45, None, sigma=3., offset_size=1.5, offset_pix=0., min_radius=5., step = 1., FWHM=3., npixels=5, output_mask_file='inner_mask.fits', output_reg_file='inner_mask.reg')
