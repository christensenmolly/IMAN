...
PATH_TO_MODULE = Path(os.path.dirname(__file__))
#sys.path.append(PATH_TO_MODULE.split('/IMAN')[0]+'/IMAN/detect_objects')



# Get access to the compiled C maxtree library
# Defaults to float version
mto_lib = ct.CDLL(PATH_TO_MODULE / 'lib/mt_objects.so')
...
def init_double_filtering(params):
    ...
    if params.d_type == ct.c_double:
        mto_lib = ct.CDLL(PATH_TO_MODULE / 'lib/mt_objects_double.so')
