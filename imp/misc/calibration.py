...
sys.path.append(os.path.join(IMAN_DIR, 'detect_objects/Sextractor'))
        ...
        if Table is None:
            ...
            for i in range(len(table)):
                ...
                if catalog=='NOMAD':
                  ...
                  B,V,R,I = find_zp.transformation_from_sdss_to_BVRI(u_mag,g_mag,r_mag,i_mag,z_mag)
                  MAG.append(float(find_zp.get_mag(u_mag,g_mag,r_mag,i_mag,z_mag,B,V,R,I,band)))
           ...
           for y in range(yMin, yMax+1):
                  ...
                  if dPoint < dEll and mask[y,x]==0.:
                    try:
                      ...
                      zz=1
        mean, median, std = sigma_clipped_stats(I_back, sigma=3.0, iters=5)
        return median, std
...
def main(input_image, output_image, band='R',catalogue='NOMAD', m0=None, sex_cat=None, region_file=None, peaks_file=None, mag_min=15.6, mag_max=18.3):
  print('Using catalogue %s' % (catalogue))
  # Open the inout image:
  ...
  if note=='*':
    print('WCS is not found. Exiting ...')
    exit()
  else:
    print('Resolution is: %.3f arcsec/pix' % (pix2sec))
  ...
  if sex_cat==None or os.path.exists(sex_cat)==False:
    # Run Sextractor
    sextr.run_sextr(input_image,20.,GAIN,pix2sec,fwhm,se_file='default_fwhm.sex')
    #run_SExtractor.call_SE(input_image, snr=2., min_pix=5)
    sex_cat = 'field.cat'
  ...
  if region_file==None or os.path.exists(region_file)==False:
	  ...
	  if class_star[k]<=cl_med+cl_std and class_star[k]>=cl_med-cl_std and flux_radius99[k]<=rad_med+rad_std and flux_radius99[k]>=rad_med-rad_std:
        ...
        for k in range(len(class_star)):
          if class_star[k]>=class_star_lim:
            cl_stars.append(k)  

        print('Number of good stars after class_star filter: %i' % (len(cl_stars)))
        ...
        print('FWHM [pix] = %.3f+/-%.3f' % (fwhm_med,fwhm_std)) 
        print('Ellipticity = %.3f+/-%.3f' % (ell_med,ell_std)) 
        ...
        print('Number of good stars after FWHM filter: %i' % (len(fwhm_stars)))
        ...
        print('Number of good non-overlaping stars away from the frame borders: %i' % (len(BEST_STARS)))
        ...
        f_reg.close()
        print('Number of found stars in the region file is: %i' % (len(BEST_STARS)))
  ...
  while N_try<=2:
    ...
    print("Zero-point for each star:")
    ...
    if N_try==2:
        repeat=True
        print('WARNING: The list of best PSF stars is empty! Using good PSF stars.')
    if VERY_FINAL_STARS!=[]:
        N_try=3
    #print(N_try, repeat)  
  ...
  #print(repeat)
  if not repeat:
    filtered_data = sigma_clip(M0, sigma=2, iters=5)
  ...
  for k in range(len(VERY_FINAL_STARS)):
    ...
    print("Star #%i: %.5f %.3f %.2f %.1f %s" % (K+1,M0[k],FWHMS[k],ELLS[k],PAS[k],comment_star))
  ...
  if m0 is None:
    m0_mean, m0_median, m0_std = sigma_clipped_stats(M0, sigma=2, iters=5)
    fwhm_mean, fwhm_med, fwhm_std = sigma_clipped_stats(FWHMS, sigma=2, iters=5)
    ell_mean, ell_med, ell_std = sigma_clipped_stats(ELLS, sigma=2, iters=5)
    theta_mean, theta_med, theta_std = sigma_clipped_stats(PAS, sigma=2, iters=5)
    
    print('FINAL NUMBER OF PSF STARS IS: %i' % (len(M0)))
    print("PSF:")
    print('\tFWHM [pix] = %.3f%s +/- %.3f' % (fwhm_med,comment,fwhm_std)) 
    print('\tEllipticity = %.3f%s +/- %.3f' % (ell_med,comment,ell_std)) 
    print('\tP.A. = %.3f%s +/- %.3f' % (theta_med,comment,theta_std)) 


    print("Photometric calibration:")
    print("\t Zero-point [mag] %.5f%s +/- %.5f" % (m0_median,comment,m0_std))
...
band = 'R'
catalogue = "NOMAD"
main(input_image,band,catalogue)#, sex_cat='field.cat')
'''