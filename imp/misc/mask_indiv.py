...
# Remove object from region file which lie within the ellipse which describes the outer isophote of the galaxy studied:
...
def find_ellipse_around(data, xc, yc, Radius):
      ny,nx = np.shape(data)
      xmin = int(np.max([xc - Radius,0.]))
      ymin = int(np.max([yc - Radius,0.]))
      xmax = int(np.min([xc + Radius,nx]))
      ymax = int(np.min([yc + Radius,ny]))
  
      props = data_properties(data[ymin:ymax,xmin:xmax])
      ...
      return float(tbl['semimajor_axis_sigma'][0]),float(tbl['semiminor_axis_sigma'][0]),np.degrees(float(tbl['orientation'][0]))

def backgr_around_star(scidata,xc,yc,R):
  nx, ny =scidata.shape[1], scidata.shape[0]
  Rin = int(math.ceil(1.1*R))
  Rout = int(math.ceil(2.*R))
  backgr = []
  X = []; Y = []; I = []
  ...
  g_init = models.Gaussian2D(amplitude=Imax, x_mean=xc, y_mean=yc, x_stddev=R/4., y_stddev=R/4., theta=0.)
  f = fitting.LevMarLSQFitter()
  mean, median, std = sigma_clipped_stats(backgr, sigma=3.0, iters=5)
  p = f(g_init, X, Y, I-median)
  return float(p.x_mean[0]),float(p.y_mean[0]),float(p.x_stddev[0]),float(p.y_stddev[0]),float(p.theta[0])

def additional_mask(input_image,xc=None,yc=None,FWHM=3.,output_mask='add_mask.reg',sigma_numb=10.0,xmin=None,ymin=None,xmax=None,ymax=None,box_size=5,median=None,std=None, galaxy_polygon=None, masking=None):
    from astropy.stats import sigma_clipped_stats
    ...
    mask_astropy = np.zeros_like(data, dtype=bool)
    ny,nx = np.shape(data)
    for k in range(ny):
      for i in range(nx):
	if data[k,i]==0:
    ...
    if xmin==None and ymin==None and xmax==None and ymax==None:
	tbl = find_peaks(data, threshold, box_size=box_size, subpixel=False)
	xmin = 0; ymin = 0
    else:
	#print 'here'
	tbl = find_peaks(data[int(math.ceil(ymin)):int(math.ceil(ymax)),int(math.ceil(xmin)):int(math.ceil(xmax))], threshold, box_size=box_size, subpixel=False, mask=mask_astropy)      
	...
	if xc==None or yc==None:
	  for k in range(len(tbl)):
	    x = tbl['x_peak'][k]
	    y = tbl['y_peak'][k]
	    f.write('circle(%f,%f,%f) # color=red\n' % (x+1+xmin,y+1+ymin,3.*FWHM))
	else:
	  for k in range(len(tbl)):
	    x = tbl['x_peak'][k]
	    y = tbl['y_peak'][k]
	    if math.sqrt((xc-(x+xmin))**2+(yc-(y+ymin))**2)>=3.*FWHM:
	      f.write('circle(%f,%f,%f) # color=red\n' % (x+1+xmin,y+1+ymin,3.*FWHM))
	f.close()
    else:
	f = open(output_mask,'w')
	if masking=='outer':
	  for k in range(len(tbl)):
	    x = tbl['x_peak'][k]
	    y = tbl['y_peak'][k]
	    if not galaxy_polygon.contains(Point(x, y)):
	      f.write('circle(%f,%f,%f) # color=red\n' % (x+1+xmin,y+1+ymin,3.*FWHM))
	  f.close()
	else:
	  for k in range(len(tbl)):
	    ...
	    if galaxy_polygon.contains(Point(x, y)):
	      f.write('circle(%f,%f,%f) # color=red\n' % (x+1+xmin,y+1+ymin,3.*FWHM))
	  f.close()
	    
def final_mask(input_image, segm_file, sex_catalog, output_region_file, xc,yc, output_image=None, output_mask=None, mode='a',rescale=None,rescale_value=None,check_mask=True,create_masks=True, FWHM=3.,sigma_numb=10.0, type_of_masking='non_poly',sky_level=None,sky_std=None, show_running=True):
    ...
    '''
    # Pre-masking:
    mask(input_image, output_region_file, factor=1, output_image='galaxy_clean_tmp.fits', output_mask='add_mask_tmp.fits')
    additional_mask('galaxy_clean_tmp.fits',xc=xc,yc=yc,FWHM=FWHM,output_mask='add_mask.reg',sigma_numb=sigma_numb,box_size=5,median=sky_level,std=sky_std)

    os.remove('galaxy_clean_tmp.fits')
    os.remove('add_mask_tmp.fits')
    #additional_mask(input_image,xc=xc,yc=yc,FWHM=FWHM,output_mask='add_mask.reg',sigma_numb=sigma_numb,box_size=5,median=sky_level,std=sky_std)
...
if __name__ == '__main__':
    ...
    mask(input_image, reg_file, output_image=output_image, output_mask=output_mask)
