...
def main(input_image, region_file, mask_file, bands=['R'], catalogues=['NOMAD'], star_number=None, obs_wavelength=0.655, ext_map='Arenou', verbosity=True, N_phot_stars=9999):
        # WARNING: At the moment, colour is not taken into account, so this parameters should be omitted.
        
        if verbosity: print('Photometric calibration by the region file with stars...')
        ...
        for k in range(Number_of_lines):
           # Do each star:
           if verbosity: show_complete(k+1, Number_of_lines) # Progress status
           ...
           ZP.append(zp)
           if len(ZP)+1>N_phot_stars:
               break
        
        if ZP==[]:
            if verbosity: print('No match of the selected stars with the catalogue. Check WCS or the field does not have good PSF stars! ')
            return None
            #exit()
        ...
        for kk in range(len(catalogues)):
               m0_mean, m0_median, m0_std = sigma_clipped_stats(results['m0:%s:%s:%s' % (catalogues[kk],bands[kk],str(None))], sigma=2)
               if m0_median==0.0:
                  m0_median = float('nan')
                  m0_std = float('nan')
               if verbosity: print('%s:%s:%s  %f %f' % (catalogues[kk],bands[kk],str(None),m0_median, m0_std))
        ...
        if verbosity: print('Number of stars used: %i' % (RESULTS['N_stars_calibr']))
