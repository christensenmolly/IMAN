#!/usr/bin/python
# DESCRIPTION:
# More sophisticated script to estimate sky background using different approaches. No mask is required.
# MINIMAL USAGE: python backEst.py [input_image]
# EXAMPLE: python backEst.py frame-g-003813-1-0201.fits --method annulus --sky_subtr yes --x_obj 179.63042 --y_obj 43.947405 --manual yes
import sys
import subprocess
...
###################
# IMP SETUP
N_box = IMP_SETUP.N_box
N_tr = IMP_SETUP.N_tr
Box_size = IMP_SETUP.Box_size
Cov_coeff = IMP_SETUP.Cov_coeff
#Annulus_coeff = IMP_SETUP.Annulus_coeff
Number_of_an_pixels = IMP_SETUP.Number_of_an_pixels
R_find = IMP_SETUP.R_find
###################
...
def fit_polynomial(data, degree, mask=None):#, x_min=None, x_max=None, y_min=None, y_max=None):

    ySize,xSize = np.shape(data)
    '''
    if x_min is None or x_min is None or y_min is None or y_max is None:
          x_min = 0
          y_min = 0
          x_max = xSize
          y_max = ySize
...
def ellipse_annulus(output_image, inner_ellipse, outer_ellipse, inframe, mask, xSize, ySize, header, sky_subtr, degree, show_running=True):
        ...
        for x in range(xMin, xMax):
            for y in range(yMin, yMax):
                ...
                if dPoint_out < dEll_out and dPoint_in > dEll_in and mask[y,x]==0. and np.isnan(inframe[y,x])==False:
                  try:
        ...
        outHDU.writeto('sky.fits',clobber=True)  
        return sky_median,std



def sky_in_ell_annulus(input_image, output_image, mask_image, ell_pars, Npix=100000, sky_subtr=True, manual=False, degree=0, annulus_width=None, show_running=True):  
    ...
    sky,std = ellipse_annulus(output_image,inner_ellipse, outer_ellipse, data_image, data_mask, xSize, ySize, header, sky_subtr, degree, show_running=show_running)
    return sky,std
...
def flat_sky_subtr(input_image):
    ...
    while ii<N_tr:
        ...
        while k<N_box:
                ...
                for m in y:
                    for n in x:
                          if mask_infr[m,n]==0. and np.isnan(scidata[m,n])==False:
...
def call_SE(fitsFile, both=True, sextr_setup=None, models=False, snr=2., min_pix = 5):
    ...
    if subprocess.call("which sex >/dev/null", shell=True) == 0:
        ...
        exit(1)
    if both==True and sextr_setup==None:
        # Cold catalogue
        if models==False:
          callString += "-c %s/Sextractor/cold.sex -DETECT_THRESH %f -ANALYSIS_THRESH %f -DETECT_MINAREA %f -PARAMETERS_NAME %s/Sextractor/default.param -FILTER_NAME  %s/Sextractor/default.conv -STARNNW_NAME %s/Sextractor/default.nnw" % (os.path.dirname(__file__),snr,snr,min_pix,os.path.dirname(__file__),os.path.dirname(__file__),os.path.dirname(__file__))
        else:
          callString += "-c %s/Sextractor/cold_objects.sex -DETECT_THRESH %f -ANALYSIS_THRESH %f -DETECT_MINAREA %f -PARAMETERS_NAME %s/Sextractor/default.param -FILTER_NAME  %s/Sextractor/default.conv -STARNNW_NAME %s/Sextractor/default.nnw" % (os.path.dirname(__file__),snr,snr,min_pix,os.path.dirname(__file__),os.path.dirname(__file__),os.path.dirname(__file__))
        ...
        # Hot catalogue
        if models==False:
          CallString += "-c %s/Sextractor/hot.sex" % (os.path.dirname(__file__))
        else:
          CallString += "-c %s/Sextractor/hot_objects.sex" % (os.path.dirname(__file__))
        CallString += " -VERBOSE_TYPE=QUIET"
    ...
    elif both==False and sextr_setup==None:
        # Cold catalogue
        if models==False:
          callString += "-c %s/Sextractor/cold.sex  -DETECT_THRESH %f -ANALYSIS_THRESH %f -DETECT_MINAREA %f -PARAMETERS_NAME %s/Sextractor/default.param -FILTER_NAME  %s/Sextractor/default.conv -STARNNW_NAME %s/Sextractor/default.nnw" % (os.path.dirname(__file__),snr,snr,min_pix,os.path.dirname(__file__),os.path.dirname(__file__),os.path.dirname(__file__))
        else:
        ...
        callString += "-c %s/Sextractor/%s" % (os.path.dirname(__file__),sextr_setup)
        callString += " -VERBOSE_TYPE=QUIET"
        subprocess.call(callString, shell=True)      
...
def fitness_function(data, poly, coeffs, diagram):
    ...
    yCen = int(ySize/2.0)
    xCen = int(xSize/2.0)
    coords = meshgrid(range(ySize), range(xSize))
...
def imp_skysub_manual(input_image, degree):
    import imp_skysub
    # If given in command, change values for fitting and plotting
    linear, plot = True, False

    if not os.path.exists("backgr.reg"):
      open("backgr.reg", "w").close()
      p = subprocess.Popen(["ds9",input_image,"-scale","histequ","-invert"])
    else:
      p = subprocess.Popen(["ds9",input_image,"-scale","histequ","-invert","-regions","load","backgr.reg"])
    p.wait()

    output_image = 'back_clean.fits'
    hdulist = pyfits.open(input_image)
    primhdr = pyfits.getheader(input_image, 0)
    frame = hdulist[0].data
    sframe,med_sky,std = imp_skysub.subtractBackground(input_image,frame,
                            "backgr.reg",degree,linear, plot)
    hdu = pyfits.PrimaryHDU(sframe,primhdr)
    hdu.writeto(output_image,clobber=True)
    return med_sky,std

def fast_background(input_image, cleanName, degree):
    from astropy.stats import sigma_clipped_stats
    ...
    mean, median, std = sigma_clipped_stats(scidata, sigma=3.0, iters=10, mask=mask_astropy)
    subtract_back(input_image, cleanName, 0, median, std, True)
    return median, std       

 
def estimate_sky(fitsFileName, cleanName = "back_clean.fits", method = 'random', degree=2, sky_subtr=True, x_obj = None, y_obj = None, coords = 'world', manual=False, reg_mask=None, ret_only_sky=True, annulus_width=None, inner_ellipse=None, Annulus_coeff=None, show_running=True):
    ...
    #          'boxes' - use boxes in ds9 to select sky
    ...
    if method=='kd-tree':
        ...
        if reg_mask==None:
          ...
          import mask_objects
          mask = mask_objects.main_mask_objects(fitsFileName,reg_mask)
        ...
        if inner_ellipse==None:
          xc,yc,sma,smb,theta = find_object(x_obj,y_obj)
        else:
          inner_ellipse = np.array(inner_ellipse.split(','), float)
          sma,smb,theta = inner_ellipse[0],inner_ellipse[1],inner_ellipse[2]
          xc = x_obj; yc = y_obj
        if np.isnan(xc)==True:
          if show_running==True:
        ...
        sky,std = sky_in_ell_annulus(fitsFileName,cleanName,'segm.fits',ell_pars,Number_of_an_pixels, sky_subtr, manual, degree, annulus_width, show_running)
    elif method=='boxes':
        sky,std = imp_skysub_manual(fitsFileName, degree)
    elif method=='fast':
        sky,std = fast_background(fitsFileName, cleanName, degree)      
    elif method=='poly':
      if reg_mask!=None:
        import mask_indiv
        mask_indiv.mask(fitsFileName, reg_mask, factor=1, output_image=None, output_mask='mask.fits', obj_ellipse=None, mask_value=1, show_running=True, mask_DN=None)
      else:
        shutil.copy('segm.fits','mask.fits')
      #exit()
      HDU_mask = pyfits.open('mask.fits')
      mask = HDU_mask[0].data      

      from astropy.stats import sigma_clipped_stats
      hdulist = pyfits.open('mask.fits')
      mask_infr = hdulist[0].data
      mask_astropy = (mask_infr>0.)

      mean, median, std = sigma_clipped_stats(data, sigma=3.0, iters=5, mask=mask_astropy)
      
      sky = fit_polynomial(data, degree, mask=mask_astropy)


      header['Sky_med'] = median
      header['Sky_std'] = std      
      outHDU = pyfits.PrimaryHDU(data-sky, header=header)
      outHDU.writeto(cleanName, clobber=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", nargs='?', const=1, help="Optional: Specify the method: random, kd-tree, annulus, fast",type=str,default='random')
    ...
    parser.add_argument("--reg", nargs='?', const=1, help="Optional: Region with all masked objects",type=str,default='none')
    parser.add_argument("--annulus_width", nargs='?', const=1, help="Optional: Only for the annulus - the width of the annulus in pix.",type=str,default=None)
    parser.add_argument("--inner_ellipse", nargs='?', const=1, help="Optional: Only for the annulus - sma,smb,PA",type=str,default=None)
    ...
    inner_ellipse = args.inner_ellipse

    sky = estimate_sky(input_image, output_image, method, degree, sky_subtr, x_obj, y_obj, coords, manual, reg_mask, True, annulus_width, inner_ellipse)
