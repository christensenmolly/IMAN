...
#  Modified: Aleksandr Mosenkov
#  python /home/amosenko/MyGit/IMAN/misc_funcs/sdss_downloader.py ugriz -i coordinates1.dat -a -s -c -f -t --ps --scatter
...
def parse_psf(psField_file, galName, band):
    print('Parsing psField: %s band' % (band))
    dir = psField_file.split(psField_file.split('/')[-1])[0]
    if band=='u':
        band_number = 1
    if band=='g':
        band_number = 2
    ...
    if band=='z':
        band_number = 5
    try:    
        subprocess.call("read_PSF %s %i 1024 744 %spsf_tmp.fits" % (psField_file, band_number, dir), shell=True)
    except:
        print('Install read_PSF from https://www.sdss.org/dr14/algorithms/read_psf/ and add it to usr/bin or usr/local/bin or to your PATH')
        exit()
    hdulist1 = pyfits.open('%spsf_tmp.fits' % (dir))
    data = hdulist1[0].data - 1000.
    header = hdulist1[0].header
    new_data = data/np.sum(data)


    hdu = pyfits.PrimaryHDU(new_data, header)
    hdu.writeto('%s%s_psf_%s.fits' % (dir, galName, band), overwrite=True)
    os.remove('%spsf_tmp.fits' % (dir))
    return os.path.abspath('%s%s_psf_%s.fits' % (dir, galName, band))
...
parser.add_argument("--skysub", action="store_true", default=False,
                    help="Subtract sky for each field")
...
with open("fields.dat", "w", buffering=0) as outFile:
    ...
    for line in listOfCoords:
        ...
        if args.skysub:
            ...
            sys.path.append('/home/amosenko/MyGit/IMAN/imp/sky_fitting')
            import auto_masking
            import convert_reg_to_mask
            import determine_sky
            ...
            for band in bandlist:
                ...
                for input_image in listOfImages:                    
                    #mto_gain = IMP.IMP_main(input_image, 'image_masking', region_file = input_image.split('.fits'[0]+'_mask.reg'), mask_image=input_image.split('.fits'[0]+'_mask.fits'), user_interact=False, verbosity=True, masking_tool='sextractor', offset_size=2.0)

                    bunzip("%s.bz2" % (input_image))
                    
                    output_region_file = auto_masking.main(input_image, output_region_file=input_image.split('.fits')[0]+'_mask.reg', snr=2., min_pix=5, region_type='polygon', sextr_setup=None, galaxy_ellipse=None, offset_size=2.0, offset_pix=0., verbosity=True)
                    output_image_tmp, output_mask_tmp = convert_reg_to_mask.mask(input_image, input_image.split('.fits')[0]+'_mask.reg', output_image='clean.fits', output_mask= input_image.split('.fits')[0]+'_mask.fits', mask_value=1, show_running=True, mask_DN=None)
                    
                    
                    
                    #output_image, mean, median, std, sky_degree = IMP.IMP_main(input_image, 'sky_correction', mask_image=input_image.split('.fits'[0]+'_mask.fits'), output_image='tmpp.fits', sky_degree=0, user_interact=False, verbosity=True, sigma_smooth=None)
                    
                    output_image, mean, median, std = determine_sky.sky_subtraction(input_image, input_image.split('.fits')[0]+'_mask.fits', polynomial_degree=0, output_image='tmpp.fits', output_sky=None, hdu_inp=0, sampling_factor=None, sigma_smooth=None, verbosity=True)
                    
                    
                    
                    move('tmpp.fits', input_image)
                    os.remove(input_image.split('.fits')[0]+'_mask.reg')
                    os.remove(input_image.split('.fits')[0]+'_mask.fits')
        '''
        #
            ...
            for band in bandlist:
                ...
                if args.skysub:
                    for input_image in listOfImages:                    
                        ...
                        output_region_file = auto_masking.main(input_image, output_region_file=input_image.split('.fits')[0]+'_mask.reg', snr=2., min_pix=5, region_type='polygon', sextr_setup=None, galaxy_ellipse=None, offset_size=3.0, offset_pix=0., verbosity=True)
                        output_image_tmp, output_mask_tmp = convert_reg_to_mask.mask(input_image, input_image.split('.fits')[0]+'_mask.reg', output_image='clean.fits', output_mask= input_image.split('.fits')[0]+'_mask.fits', mask_value=1, show_running=True, mask_DN=None)
                        ...
                        output_image, mean, median, std = determine_sky.sky_subtraction(input_image, input_image.split('.fits')[0]+'_mask.fits', polynomial_degree=1, output_image='tmpp.fits', output_sky=None, hdu_inp=0, sampling_factor=None, sigma_smooth=None, verbosity=True)
                        ...
                        for file in ['clean.fits','field.cat', 'mask_tmp.reg', 'segm.fits']:
        ...
        if args.trim and (not wcsOK):
            ...
            for band in bandlist:            
                ...
                hdu.close()
                '''
                crop_image.main(fName, xCropMin, yCropMin, xCropMax, yCropMax, output_image=fOutName, hdu=0)
                if args.free:
                    os.remove(fName)
        ...
        if args.scatter:
            ...
            for src in fileList:
                for band in bandlist:
                    file = parse_psf(src, galName, band)
            os.remove(src)
