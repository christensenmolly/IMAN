...
def main(input_image,m0,pix2sec):
      ...
      for layer in layers:
	    ...
	    if layer==1:
	      ...
	      min_I = m0 - 2.5*log10(I)+ 5.*log10(pix2sec)
	      ...
	      max_I = m0 - 2.5*log10(i1)+ 5.*log10(pix2sec)
	      max_mag = np.max(max_I[~np.isnan(max_I)])
	      ...
	      r1 = np.array(r1)
	      i1 = np.array(i1)      
	      plot(r1*pix2sec, m0 - 2.5*log10(i1)+ 5.*log10(pix2sec),'r-',color=color2,lw=2)        
